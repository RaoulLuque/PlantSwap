...
def test_create_trade_request_from_plant_ids_outgoing_plant_does_not_exist(db: Session):
...
def test_create_trade_request_from_plant_ids_incoming_plant_does_not_exist(db: Session):
    with create_random_plant(db) as (_, _, plant):
        with pytest.raises(ValueError) as e:
            requests_crud.create_trade_request_from_plant_ids(
...
def test_get_all_trade_requests_outgoing_and_incoming_only(db: Session):
